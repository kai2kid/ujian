...
<?php
	...
	if ($_REQUEST['modeProses']=='INSERT'){
		...
		if ($baris)
		{
			...
			{
				if (isset($_REQUEST["mapel_".$baris["KODE_MAPEL"]]))
				{
					$kueri = "INSERT INTO MAPEL_USER (KODE_MAPEL, USERNAME) VALUES ('".$baris["KODE_MAPEL"]."', '".$_REQUEST["edUser"]."')";
					mysql_query($kueri);
				}
			}
		...
		$query = "DELETE FROM MAPEL_USER WHERE USERNAME='".$_REQUEST["edUser"]."'";
		mysql_query($query);
